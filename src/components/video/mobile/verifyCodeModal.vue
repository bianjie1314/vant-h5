        ...
        <div class="modal-layer-popup">
            <div class="modal-layer-success"
                ...
                <div class="modal-layer-popup-content">
                    此验证码将于<span>{{refreshTime}}</span>秒后过期<br/>用普通话朗读数字，视频时长<span>3-8</span>秒最佳
                </div>
                <div class="modal-layer-popup-number">
                    <span v-for="(items,index) in codeResult" :key="index">{{items}}</span>
                </div>
        ...
        methods: {
            ...
            getVerifyCodeModal() {
                let formdata = {
                    ...
                    service: 'user.lineBodyCheck',
                    orderCode: this.orderCode,
                    serialNumber:this.serialNumber
                };

                if(this.simcheck != null){
                    formdata.simcheck = this.simcheck;
                }

                this.$api.post("/mstx/pr/gateway.do", formdata, (res) => {
                    let res2 = JSON.parse(res).result;
                    if (res2) {
                        this.codeResult = res2.code;
...
<style lang="less">
    ...
    .modal-layer {
        ...
        &-close {
            ...
            background: url(../../../assets/video/icon_close.png) 0 0 no-repeat;
        ...
        &-fail {
            ...
            &-icon {
                ...
                background: url(../../../assets/video/icon_fail.png) 0 0 no-repeat;
                background-size: 100% auto;
            }
